...
<head>
  ...
  <title>SimVideo - Videoclipuri apreciate</title>
      ...
      <div class="main-content">
        <section class="section">
          <div class="section-header">
            <h1>Videoclipuri apreciate</h1>
          </div>
            <div class="row">
              <?php  
                $myid = $user['id'];
                $sql_videoclipuri = "SELECT * FROM aprecieri WHERE id_utilizator = '$myid' ORDER BY id DESC";
                $result_videoclipuri = mysqli_query($db, $sql_videoclipuri);
                $videoclipuri = mysqli_fetch_all($result_videoclipuri, MYSQLI_ASSOC);
                foreach($videoclipuri as $videoclip_ap):
              ?>
              <?php  
                  $id_video = $videoclip_ap['id_videoclip'];
